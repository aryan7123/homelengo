...
const page = () => {
  ...
  const fetchTotalReviews = async () => {
    try {
      const req = await axios.post("/api/add-total-reviews", {
  ...
  const filteredProperties = properties.filter(
    (property) =>
      ...
      (selectStatus === "" ||
        selectStatus === "All" ||
        property.status === selectStatus)
  );

  const filteredStatusProperties =
    properties && properties.filter((item) => item.status === "Pending");
  ...
  const deleteProperty = async (propertyId: number) => {
    try {
      const response = await axios.delete("/api/delete-property", {
        data: { id: propertyId },
      });
        ...
        <div className="w-full flex md:flex-row flex-col md:items-center items-start gap-6">
          <div className="w-full flex items-center md:justify-center justify-start gap-3 md:w-1/4 shadow rounded-2xl bg-white p-5">
            ...
            <div className="flex flex-col gap-2">
              <h3 className="text-[#5c6368] font-semibold text-base">
                Your Listings
              </h3>
              <span className="text-[#161e2d] font-semibold text-4xl">
                {properties?.length}
              </span>
            </div>
          </div>
          <div className="w-full flex items-center md:justify-center justify-start gap-3 md:w-1/4 shadow rounded-2xl bg-white p-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <span className="text-[#161e2d] font-semibold text-4xl">
                {filteredStatusProperties?.length}
              </span>
            </div>
          </div>
          <div className="w-full flex items-center md:justify-center justify-start gap-3 md:w-1/4 shadow rounded-2xl bg-white p-5">
