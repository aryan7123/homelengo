...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      {/* sidebar */}
      <aside
        className={`fixed md:block hidden dashboard-sidebar w-72 bg-[#161e2d] ${
          showSidebar ? "translate-x-[-100%]" : "translate-x-0"
        } transition-transform delay-150 top-0 left-0 bottom-0 h-screen z-[1000] overflow-y-auto overflow-x-hidden scroll-smooth`}
      >
        <div className="w-[inherit] py-8 px-6 border-b border-[#ffffff0d]">
          <Link href="/">
            <Image
              src="/logo/logo-footer@2x.png"
              width={160}
              height={80}
              alt="logo"
            ></Image>
        ...
        <div className="w-[inherit] py-8 px-6 border-b border-[#ffffff0d]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <small className="text-white truncate max-w-[150px] block">
                {session?.user?.email}
              </small>
            </div>
          </div>
          ...
          <ul className="mt-4">
            <li className="mb-2">
              <Link
                href="/dashboard"
                className={`flex ${
                  pathname === "/dashboard" ? "bg-[#1563df]" : "bg-transparent"
            ...
            <li className="mb-2">
              <Link
                href="/dashboard/my-favoruites"
                className={`flex ${
                  pathname === "/dashboard/my-favoruites"
                    ? "bg-[#1563df]"
                    : "bg-transparent"
                } items-center gap-2.5 transition-colors hover:bg-[#1563df] text-white px-6 py-4`}
              >
            ...
            <li className="mb-2">
              <button
                ...
                className={`w-full flex items-center gap-2.5 text-white px-6 transition-colors hover:bg-[#1563df] py-4`}
              >
                <LogOutIcon />
                <span>Logout</span>
              </button>
      ...
      <nav className="bg-white md:block hidden sticky top-0 left-0 w-full shadow-md z-[999]">
        <div className="max-w-7xl mx-auto transition-all flex items-center justify-between md:px-0 px-6 h-20">
          <ul
            className={`md:flex hidden items-center justify-between gap-10 ${
              showSidebar ? "pl-0" : "md:pl-[230px]"
            }`}
          >
            ...
            <li
              className={`text-base font-medium relative hover:text-[#1563df] transition-colors`}
            >
              <Link href="/">Home</Link>
            </li>
            <li
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="flex items-center">
                <Image
                  src={session?.user?.avatar || "/avatar/circle-user-round.svg"}
                  width={20}
                  height={20}
